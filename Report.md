...
4.  Look at the following code:
   ...
   4a. Would the code run correctly? Even if it does compile, what would be some potential runtime issues? 
   
   4b. After answering your thoughts, put the output of a run below (you may need to run it a few times).
   
   ```text
   output here
   ```
   
   4c. Fix the code in the following block:
